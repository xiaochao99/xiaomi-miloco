    ...
    if (!tool_calls.empty()) {
        ...
        for (const auto& tc : tool_calls) {
            arr.push_back({
                ...
                {"id", tc.id},
                // // Some templates generate and require an id (sometimes in a very specific format, e.g. Mistral
                // Nemo).
                // // We only generate a random id for the ones that don't generate one by themselves
                // // (they also won't get to see it as their template likely doesn't use it, so it's all for the
                // client)
...
std::vector<common_chat_msg_diff> common_chat_msg_diff::compute_diffs(const common_chat_msg& previous_msg,
...
std::string decode_base64(std::string mes) {
    const std::string prefix = "data:image/jpeg;base64,";
    if (mes.rfind(prefix, 0) == 0) {
        mes = mes.substr(prefix.size());
    }

    try {
        return base64::decode(mes);
                ...
                if (content.is_string()) {
                    ...
                    for (const auto& part : content) {
                        ...
                        if (type == "text") {
                            ...
                            msg_part.images.push_back(decode_base64(part.at("image").get<std::string>()));
                        } else if (type == "video") {
                            for (const auto& image_b64 : part.at("video")) {
                                msg_part.images.push_back(decode_base64(image_b64.get<std::string>()));
                            }
                        }
        ...
        if (!msg.content.empty()) {
            ...
            if (concat_typed_text) {
                ...
                for (const auto& part : msg.content_parts) {
                    if (part.type != "text" && part.type != "video" && part.type != "image") {
                        LOG_WRN("Ignoring non-text/video/image content part: %s\n", part.type.c_str());
                        continue;
                    }
                    if (part.type == "image") {
                        text += MICO_DEFAULT_IMAGE_MARKER;
                    } else if (part.type == "video") {
                        auto images_num = part.images.size();
...
void common_chat_templates_free(struct common_chat_templates* tmpls) { delete tmpls; }
...
const char* common_chat_format_name(common_chat_format format) {
    switch (format) {
        ...
        case COMMON_CHAT_FORMAT_LLAMA_3_X_WITH_BUILTIN_TOOLS:
            return "Llama 3.x with builtin tools";
        case COMMON_CHAT_FORMAT_DEEPSEEK_R1:
            return "DeepSeek R1";
        case COMMON_CHAT_FORMAT_FIREFUNCTION_V2:
            return "FireFunction v2";
        case COMMON_CHAT_FORMAT_FUNCTIONARY_V3_2:
            return "Functionary v3.2";
        case COMMON_CHAT_FORMAT_FUNCTIONARY_V3_1_LLAMA_3_1:
            return "Functionary v3.1 Llama 3.1";
...
/**
 * Takes a prefix regex that must have 1 group to capture the function name, a closing suffix, and expects json
 * parameters in between. Aggregates the prefix, suffix and in-between text into the content.
 */
static void parse_json_tool_calls(
    ...
    const common_regex& close_regex, const std::optional<common_regex>& block_close, bool allow_raw_python = false,
...
static void parse_prefixed_json_tool_call_array(common_chat_msg_parser& builder, const common_regex& prefix,
...
static common_chat_params common_chat_params_init_mistral_nemo(const common_chat_template& tmpl,
                                                               const struct templates_params& inputs) {
    common_chat_params data;
                ...
                {"properties",
                 {
                     ...
                     // It's hard to constrain that for now (while reusing the JSON schema conversion), so we're just
                     // expecting a plain object.
                     {"name",
                      {
                          {"type", "string"},
                          {"const", function.at("name")},
...
static common_chat_params common_chat_params_init_command_r7b(const common_chat_template& tmpl,
                                                              const struct templates_params& inputs) {
    common_chat_params data;
    ...
    data.prompt =
        apply(tmpl, adjusted_messages, inputs.tools.empty() ? json() : inputs.tools, inputs.add_generation_prompt, {});
    ...
    data.grammar = build_grammar([&](const common_grammar_builder& builder) {
        ...
        builder.add_rule("root", std::string(data.thinking_forced_open ? "( \"<|END_THINKING|>\" space )? " : "") +
                                     "\"<|START_ACTION|>\" " + builder.add_schema("tool_calls", schema) +
                                     " \"<|END_ACTION|>\"");
    });
    data.grammar_triggers.push_back(
        {COMMON_GRAMMAR_TRIGGER_TYPE_PATTERN_FULL,
         // If thinking_forced_open, then we capture the </think> tag in the grammar,
         // (important for required tool choice) and in the trigger's first capture (decides what is sent to the
         // grammar)
         std::string(data.thinking_forced_open ? "[\\s\\S]*?(<\\|END_THINKING\\|>\\s*)"
                                               : "(?:<\\|START_THINKING\\|>[\\s\\S]*?<\\|END_THINKING\\|>\\s*)?") +
             "(<\\|START_ACTION\\|>)[\\s\\S]*"});
    data.preserved_tokens = {
        "<|START_ACTION|>", "<|END_ACTION|>",     "<|START_RESPONSE|>",
        "<|END_RESPONSE|>", "<|START_THINKING|>", "<|END_THINKING|>",
    };
    return data;
}
...
static void expect_tool_parameters(const std::string& name, const json& parameters,
                                   const std::vector<std::string>& expected_properties) {
    if (!parameters.is_object() || !parameters.contains("type") || parameters.at("type") != "object" ||
        !parameters.contains("properties") || !parameters.contains("required")) {
    ...
    for (const auto& prop : expected_properties) {
        ...
        if (std::find(parameters_required.begin(), parameters_required.end(), json(prop)) ==
            parameters_required.end()) {
            throw std::runtime_error("Parameters of tool " + name +
                                     " must have property marked as required: " + prop);  // NOLINT
        }
    }
    if (parameters_properties.size() != expected_properties.size()) {
        throw std::runtime_error("Parameters of tool " + name +
                                 " must only have these properties:" + string_join(expected_properties, ", "));
    }
}

static common_chat_params common_chat_params_init_llama_3_x(const common_chat_template& tmpl,
                                                            const struct templates_params& inputs,
        ...
        data.grammar = build_grammar([&](const common_grammar_builder& builder) {
            ...
            auto handle_builtin_tool = [&](const std::string& name, const json& parameters) {
                ...
                tool_rules.push_back(builder.add_rule(
                    name + "-call", "\"<|python_tag|>" + name + ".call(\" " + string_join(kvs, " \", \" ") + " \")\""));
            ...
            // Small models may hallucinate function names so we match anything (*at the start*) that looks like the
            // JSON of a function call, regardless of the name.
            data.grammar_triggers.push_back({
                COMMON_GRAMMAR_TRIGGER_TYPE_PATTERN_FULL,
                "(\\{\\s*(?:\"type\"\\s*:\\s*\"function\"\\s*,\\s*)?\"name\"\\s*:\\s*\")[\\s\\S]*",  // + name +
                                                                                                     // "\"[\\s\\S]*",
            });
...
static void common_chat_parse_llama_3_1(common_chat_msg_parser& builder, bool with_builtin_tools = false) {
    ...
    parse_json_tool_calls(builder,
                          ...
                          /* function_regex= */ std::nullopt, close_regex, std::nullopt);
}
...
static common_chat_params common_chat_params_init_deepseek_r1(const common_chat_template& tmpl,
    ...
    if (tmpl.source().find("{% if ns.is_tool %}{{'<｜tool▁outputs▁end｜>'}}") != std::string::npos) {
        ...
        prompt = std::regex_replace(
            prompt, std::regex("(<｜tool▁call▁end｜>)[\\s\\r\\n]*(<｜tool▁outputs▁begin｜>|<｜User｜>)"),
        ...
        data.grammar = build_grammar([&](const common_grammar_builder& builder) {
            ...
            foreach_function(inputs.tools, [&](const json& tool) {
                ...
                tool_rules.push_back(
                    builder.add_rule(name + "-call", "( \"<｜tool▁call▁begin｜>\" )? \"function<｜tool▁sep｜>" + name +
                                                         "\\n"
                                                         "```json\\n\" " +
                                                         builder.add_schema(name + "-args", parameters) +
                                                         " "
            ...
            builder.add_rule("root", std::string(data.thinking_forced_open ? "( \"</think>\" space )? " : "") +
                                         "( \"<｜tool▁calls▁begin｜>\" | \"<｜tool_calls_begin｜>\" | \"<｜tool calls "
                                         "begin｜>\" | \"<｜tool\\\\_calls\\\\_begin｜>\" | \"<｜tool▁calls｜>\" ) "
                                         "(" +
            ...
            data.grammar_triggers.push_back(
                {COMMON_GRAMMAR_TRIGGER_TYPE_PATTERN_FULL,
                 ...
                 std::string(data.thinking_forced_open ? "[\\s\\S]*?(</think>\\s*)"
                                                       : "(?:<think>[\\s\\S]*?</think>\\s*)?") +
                     "(<｜tool▁calls▁begin｜>|<｜tool_calls_begin｜>|<｜tool calls "
                     "begin｜>|<｜tool\\\\_calls\\\\_begin｜>|<｜tool▁calls｜>)[\\s\\S]*"});
            data.preserved_tokens = {
                "<think>",
                "</think>",
                "<｜tool▁calls▁begin｜>",
...
static void common_chat_parse_deepseek_r1(common_chat_msg_parser& builder) {
    ...
    static const common_regex tool_calls_begin(
        "(?:<｜tool▁calls▁begin｜>|<｜tool_calls_begin｜>|<｜tool calls "
        "begin｜>|<｜tool\\\\_calls\\\\_begin｜>|<｜tool▁calls｜>)");
    ...
    parse_json_tool_calls(builder,
                          ...
                          /* function_regex_start_only= */ std::nullopt, function_regex, close_regex, tool_calls_end);
}

static common_chat_params common_chat_params_init_firefunction_v2(const common_chat_template& tmpl,
                                                                  const struct templates_params& inputs) {
    LOG_DBG("%s\n", __func__);
    common_chat_params data;
...
static common_chat_params common_chat_params_init_functionary_v3_2(const common_chat_template& tmpl,
    ...
    // If the function is python, we also allow raw python code (if the line after `python\n` doesn't start w/ opening
    // `{`), which the model seems to prefer for multiline code.
    common_chat_params data;
    data.prompt =
        apply(tmpl, inputs.messages, inputs.tools.empty() ? json() : inputs.tools, inputs.add_generation_prompt);
    ...
    if (inputs.tools.is_array() && !inputs.tools.empty()) {
        ...
        data.grammar = build_grammar([&](const common_grammar_builder& builder) {
            ...
            auto first_rule =
                first_tool_rules.empty()
                    ? ""
                    : builder.add_rule("first_tool_call", string_join(first_tool_rules, " | ")) + " space";
            if (inputs.parallel_tool_calls) {
                auto subsequent_rule =
                    builder.add_rule("subsequent_tool_call", string_join(subsequent_tool_rules, " | ")) + " space";
...
static void common_chat_parse_functionary_v3_2(common_chat_msg_parser& builder) {
    ...
    parse_json_tool_calls(builder, std::nullopt, function_regex_start_only, function_regex, close_regex, std::nullopt,
...
static common_chat_params common_chat_params_init_functionary_v3_1_llama_3_1(const common_chat_template& tmpl,
                                                                             const struct templates_params& inputs) {
    ...
    if (!inputs.tools.is_null()) {
        ...
        data.grammar = build_grammar([&](const common_grammar_builder& builder) {
            ...
            foreach_function(inputs.tools, [&](const json& tool) {
                ...
                tool_rules.push_back(builder.add_rule(
                    name + "-call", "\"<function=" + name + ">\" " + builder.add_schema(name + "-args", parameters) +
                                        " \"</function>\" space"));
            });
            if (has_raw_python) {
...
static common_chat_params common_chat_params_init_hermes_2_pro(const common_chat_template& tmpl,
    ...
    data.prompt = apply(tmpl, inputs.messages, inputs.tools.empty() ? json() : inputs.tools,
                        inputs.add_generation_prompt, additional_context);
        ...
        data.grammar = build_grammar([&](const common_grammar_builder& builder) {
            ...
            foreach_function(inputs.tools, [&](const json& tool) {
                ...
                tool_call_alts.push_back(
                    builder.add_rule(name + "-function-tag", "\"<function\" ( \"=" + name + "\" | \" name=\\\"" + name +
                                                                 "\\\"\" ) \">\" space " +
                                                                 builder.add_schema(name + "-args", parameters) +
                                                                 " "
                                                                 "\"</function>\" space"));
            ...
            // Trigger on some common known "good bad" outputs (only from the start and with a json that's about a
            // specific argument name to avoid false positives)
            data.grammar_triggers.push_back({
                COMMON_GRAMMAR_TRIGGER_TYPE_PATTERN_FULL,
                // If thinking_forced_open, then we capture the </think> tag in the grammar,
                // (important for required tool choice) and in the trigger's first capture (decides what is sent to the
...
static common_chat_params common_chat_templates_apply_jinja(const struct common_chat_templates* tmpls,
    ...
    // Firefunction v2 requires datetime and functions in the context even w/o tools, so we also use its handler in all
    // cases.
...
static common_chat_params common_chat_templates_apply_legacy(const struct common_chat_templates* tmpls,
    ...
    for (const auto& msg : inputs.messages) {
        ...
        for (const auto& part : msg.content_parts) {
            ...
            if (part.type == "image") {
                content += MICO_DEFAULT_IMAGE_MARKER;
            } else if (part.type == "video") {
                auto images_num = part.images.size();
                for (size_t i = 0; i < images_num; ++i) {
                    content += MICO_DEFAULT_IMAGE_MARKER;
    ...
    if (res < 0) {
        ...
        // this is a bit redundant (for good), since we're not sure if user validated the custom template with
        // llama_chat_verify_template()
    ...
    if (!inputs.enable_thinking) {
        // todo support other templates of MiMo/qwen2.5
        params.prompt += "<think>\n</think>";
    }
