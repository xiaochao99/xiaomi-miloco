...
struct mtmd_image_tokens {
    ...
    mtmd_image_tokens clone() { return mtmd_image_tokens{nx, ny, use_mrope_pos, batch_f32.clone(), id}; }
};
...
struct mtmd_audio_tokens {
    ...
    mtmd_audio_tokens clone() { return mtmd_audio_tokens{n_tokens, batch_f32.clone(), id}; }
};
...
mtmd_context_params mtmd_context_params_default() {
    ...
    params.image_marker = MICO_DEFAULT_IMAGE_MARKER;
    params.media_marker = MICO_DEFAULT_IMAGE_MARKER;
    return params;
}
...
struct mtmd_context {
    ...
    mtmd_context(const char* mmproj_fname, const llama_model* text_model, const mtmd_context_params& ctx_params)
        ...
        std::string mmproj_fname_str(mmproj_fname);
        if (mmproj_fname_str.empty()){
            LOG_INF("mmproj_fname is empty, skipping loading mmproj\n");
            return;  // no mmpro
        }
    ...
    void init_vision() {
        ...
        if (proj == PROJECTOR_TYPE_GEMMA3) {
            ...
            img_end = "<|image_end|>";
            LOG_WRN(
                "%s: llama 4 vision is known to have degraded quality:\n"
    ...
    void init_audio() {
        ...
        LOG_WRN(
            "%s: audio input is in experimental stage and may have reduced quality:\n"
    ...
    projector_type proj_type_v() const { return ctx_v ? clip_get_projector_type(ctx_v) : PROJECTOR_TYPE_UNKNOWN; }

    projector_type proj_type_a() const { return ctx_a ? clip_get_projector_type(ctx_a) : PROJECTOR_TYPE_UNKNOWN; }
...
mtmd_context* mtmd_init_from_file(const char* mmproj_fname, const struct llama_model* text_model,
...
struct mtmd_tokenizer {
    ...
    mtmd_tokenizer(mtmd_context* ctx, const mtmd_input_text* text, const mtmd_bitmap** bitmaps, size_t n_bitmaps)
            ...
            if (part == ctx->media_marker) {
                ...
                if (i_bm >= bitmaps.size()) {
                    LOG_ERR("%s: error: number of bitmaps (%zu) does not match number of markers (%zu)\n", __func__,
                            bitmaps.size(), parts.size() - 1);
                    return 1;
                }
                const mtmd_bitmap* bitmap = bitmaps[i_bm++];
                // while add image start token and image end token
    ...
    void add_text(const std::vector<llama_token>& tokens) {
        ...
        if (!cur.entries.empty() && cur.entries.back().type == MTMD_INPUT_CHUNK_TYPE_TEXT) {
            cur.entries.back().tokens_text.insert(cur.entries.back().tokens_text.end(), tokens.begin(), tokens.end());
        } else {
            mtmd_input_chunk chunk{
                MTMD_INPUT_CHUNK_TYPE_TEXT, tokens,
                nullptr,  // image tokens
                nullptr,  // audio tokens
            };
    ...
    int32_t add_media(const mtmd_bitmap* bitmap) {
        if (!bitmap->is_audio) {
            ...
            if (ctx->slice_tmpl == MTMD_SLICE_TMPL_MINICPMV_2_5 || ctx->slice_tmpl == MTMD_SLICE_TMPL_MINICPMV_2_6 ||
                ...
                std::string id = mtmd_bitmap_get_id(bitmap);
                {
                    if (id.empty()) {
                        // Otherwise, generate hash from image data
    ...
    static std::vector<llama_token> mtmd_tokenize_text_internal(const struct llama_vocab* vocab,
        ...
        if (n_tokens < 0) {
            ...
            int check = llama_tokenize(vocab, text.data(), text.length(), result.data(), result.size(), add_special,
                                       parse_special);
...
int32_t mtmd_tokenize(mtmd_context* ctx, mtmd_input_chunks* output, const mtmd_input_text* text,
...
int32_t mtmd_encode_chunk(mtmd_context* ctx, const mtmd_input_chunk* chunk) {
    if (chunk->type == MTMD_INPUT_CHUNK_TYPE_TEXT) {
        ...
        bool ok = clip_image_batch_encode(ctx->ctx_a, ctx->n_threads, &chunk->tokens_audio->batch_f32,
...
int32_t mtmd_encode(mtmd_context* ctx, const mtmd_image_tokens* image_tokens) {
    ...
    if (clip_is_llava(ctx_clip) || clip_is_minicpmv(ctx_clip) || clip_is_glm(ctx_clip)) {
        // TODO @ngxson : llava does not support batched encoding ; this should be fixed inside
        ...
        ok = clip_image_batch_encode(ctx_clip, ctx->n_threads, &image_tokens->batch_f32, ctx->image_embd_v.data());
    }

    return ok ? 0 : 1;
}

float* mtmd_get_output_embd(mtmd_context* ctx) { return ctx->image_embd_v.data(); }
...
bool mtmd_decode_use_mrope(mtmd_context* ctx) { return ctx->use_mrope; }

bool mtmd_support_vision(mtmd_context* ctx) { return ctx->ctx_v != nullptr; }

bool mtmd_support_audio(mtmd_context* ctx) { return ctx->ctx_a != nullptr; }
...
uint32_t mtmd_bitmap_get_nx(const mtmd_bitmap* bitmap) { return bitmap->nx; }

uint32_t mtmd_bitmap_get_ny(const mtmd_bitmap* bitmap) { return bitmap->ny; }

const unsigned char* mtmd_bitmap_get_data(const mtmd_bitmap* bitmap) { return bitmap->data.data(); }

size_t mtmd_bitmap_get_n_bytes(const mtmd_bitmap* bitmap) { return bitmap->data.size(); }

bool mtmd_bitmap_is_audio(const mtmd_bitmap* bitmap) { return bitmap->is_audio; }

const char* mtmd_bitmap_get_id(const mtmd_bitmap* bitmap) { return bitmap->id.c_str(); }
...
size_t mtmd_input_chunks_size(const mtmd_input_chunks* chunks) { return chunks->entries.size(); }
...
void mtmd_input_chunks_add_chunk(mtmd_input_chunks* chunks, const mtmd_input_chunk* chunk) {
    ...
    if (copied_chunk) {
        chunks->entries.emplace_back(std::move(*copied_chunk));
    }
}
...
void mtmd_input_chunks_insert_chunk_front(mtmd_input_chunks* chunks, const mtmd_input_chunk* chunk) {
    ...
    mtmd_input_chunk* copied_chunk = mtmd_input_chunk_copy(chunk);
    if (copied_chunk) {
        chunks->entries.insert(chunks->entries.begin(), std::move(*copied_chunk));
    }
}

// mtmd_input_chunk

enum mtmd_input_chunk_type mtmd_input_chunk_get_type(const mtmd_input_chunk* chunk) { return chunk->type; }
...
size_t mtmd_image_tokens_get_n_tokens(const mtmd_image_tokens* image_tokens) { return image_tokens->n_tokens(); }

size_t mtmd_image_tokens_get_nx(const mtmd_image_tokens* image_tokens) { return image_tokens->nx; }

size_t mtmd_image_tokens_get_ny(const mtmd_image_tokens* image_tokens) { return image_tokens->ny; }

const char* mtmd_image_tokens_get_id(const mtmd_image_tokens* image_tokens) { return image_tokens->id.c_str(); }
...
mtmd_input_chunks* mtmd_create_text_chunks(std::vector<llama_token> tokens) {
    ...
    mtmd_input_chunk chunk_text{
        MTMD_INPUT_CHUNK_TYPE_TEXT, std::move(tokens),
        nullptr,  // image tokens
        nullptr,  // audio tokens
    };
...
mtmd_input_chunks* mtmd_test_create_input_chunks() {
    ...
    mtmd_input_chunk chunk_text{
        MTMD_INPUT_CHUNK_TYPE_TEXT, std::move(tokens_text),
        nullptr,  // image tokens
        nullptr,  // audio tokens
    };
    chunks->entries.emplace_back(std::move(chunk_text));
