...
void llama_sbatch::add_seq_to_ubatch(llama_ubatch& ubatch, llama_sbatch_seq& seq, size_t length) {
    ...
    if (batch->embd) {
        if (ubatch.equal_seqs) {
            for (size_t i = 0; i < length; ++i) {
                memcpy(ubatch.embd + (n_embd * (ubatch.n_tokens + i)), batch->embd + (n_embd * ids[seq.offset + i]),
                       n_embd * sizeof(float));
            }
        } else {
            // simple split
...
llama_ubatch llama_sbatch::split_equal(size_t n_ubatch) {
    ...
    if (!seq.empty()) {
        ...
        for (size_t i = seq.size(); i-- > 0;) {
            ...
            if (length + n_tokens_in_ubatch > n_ubatch) {
                break;
            }
        }
    }
...
llama_sbatch::llama_sbatch(const llama_batch& batch, size_t n_embd, bool simple_split) {
    ...
    std::sort(ids.begin(), ids.end(), [&batch](size_t a, size_t b) {
    ...
    std::sort(seq.begin(), seq.end(), [](llama_sbatch_seq& a, llama_sbatch_seq& b) {
...
bool llama_batch_allocr::init(const llama_batch& batch_inp, const llama_vocab& vocab, const llama_memory_i* memory,
                              bool embd_all) {
    clear();
        ...
        if (debug > 1) {
            ...
            for (int32_t i = 0; i < batch.n_tokens; ++i) {
                ...
                LLAMA_LOG_DEBUG("%s:  %4d: id = %6d (%16s), pos = %4d, n_seq_id = %2d, seq_id = [%s], output = %d\n",
                                __func__, i, batch.token[i], vocab.token_to_piece(batch.token[i]).c_str(), batch.pos[i],
                                batch.n_seq_id[i], ss.str().c_str(), batch.logits[i]);
            }
            LLAMA_LOG_DEBUG("%s:   ]\n", __func__);
            ...
            for (int s0 = 0; s0 < (int)seq_pos.size(); ++s0) {
                ...
                LLAMA_LOG_DEBUG("%s:  %4d: pos = [%4d, %4d], cpl = %s\n", __func__, s0, seq_pos_min(s0),
                                seq_pos_max(s0), ss.str().empty() ? "-" : ss.str().c_str());
            }
            LLAMA_LOG_DEBUG("%s:   ]\n", __func__);
        }
    }
    ...
    for (int32_t s = 0; s < LLAMA_MAX_SEQ; ++s) {
        ...
        if (memory && seq_pos_min(s) != memory->seq_pos_max(s) + 1) {
            LLAMA_LOG_ERROR(
                "%s: sequence %d postion %d does not start(+1) from the last position %d stored in the memory\n", __func__,
                s, seq_pos_min(s), memory->seq_pos_max(s));
            return false;
        }
...
const llama_batch& llama_batch_allocr::get_batch() const { return batch; }

uint32_t llama_batch_allocr::get_n_outputs() const { return n_outputs; }
