...
class TriggerRuleDAO:
    ...
    def create(self, rule: TriggerRule) -> Optional[str]:
        ...
        try:
            ...
            sql = """
                INSERT INTO trigger_rule (id, name, enabled, camera_dids, ha_devices, condition, condition_type, ha_condition, trigger_entity_id, detection_condition, execute_info, filter, created_at, updated_at)
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
            """
    ...
    def update(self, rule: TriggerRule) -> bool:
        ...
        try:
            sql = """
                UPDATE trigger_rule
                SET name = ?, enabled = ?, camera_dids = ?, ha_devices = ?, condition = ?, condition_type = ?, ha_condition = ?, trigger_entity_id = ?, detection_condition = ?, execute_info = ?, filter = ?, updated_at = ?
                WHERE id = ?
            """
            params = (
                rule.name,
