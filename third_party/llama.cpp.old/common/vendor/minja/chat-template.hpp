...
private:
    ...
    std::string try_raw_render(const nlohmann::ordered_json& messages, const nlohmann::ordered_json& tools,
...
public:
    chat_template(const std::string& source, const std::string& bos_token, const std::string& eos_token)
        ...
        const json dummy_typed_user_msg = {{"role", "user"},
                                           {"content", json::array({{{"type", "text"}, {"text", user_needle}}})}};
        ...
        const json needle_system_msg = {
            {"role", "system"},
            {"content",
             caps_.requires_typed_content ? json::array({{{"type", "text"}, {"text", sys_needle}}}) : json(sys_needle)},
        };

        caps_.supports_system_role = contains(try_raw_render(
                                                  {
                                                      needle_system_msg,
                                                      dummy_user_msg,
                                                  },
        ...
        auto out = try_raw_render(json::array({dummy_user_msg}),
                                  json::array({
                                      {
                                          {"name", "some_tool"},
                                          {"type", "function"},
        ...
        out = try_raw_render(json::array({
                             ...
                             {}, false);
        auto tool_call_renders_str_arguments =
            contains(out, "\"argument_needle\":") || contains(out, "'argument_needle':");
        out = try_raw_render(json::array({
                                 dummy_user_msg,
                                 make_tool_calls_msg(json::array({make_tool_call("ipython", dummy_args_obj)})),
                             }),
                             {}, false);
        auto tool_call_renders_obj_arguments =
            contains(out, "\"argument_needle\":") || contains(out, "'argument_needle':");

        caps_.supports_tool_calls = tool_call_renders_str_arguments || tool_call_renders_obj_arguments;
        caps_.requires_object_arguments = !tool_call_renders_str_arguments && tool_call_renders_obj_arguments;
        auto out_empty =
            try_raw_render(json::array({dummy_user_msg, {{"role", "assistant"}, {"content", ""}}}), {}, false);
        auto out_null =
            try_raw_render(json::array({dummy_user_msg, {{"role", "assistant"}, {"content", nullptr}}}), {}, false);
        ...
        if (caps_.supports_tool_calls) {
            ...
            out = try_raw_render(json::array({dummy_user_msg,
            ...
            if (!caps_.supports_tools) {
                     ...
                     json::array({
                         {
                             ...
                             {"function",
                              {
                                  {"name", "tool_name"},
                                  {"arguments", (caps_.requires_object_arguments
                                                     ? args
                                                     : json(minja::Value(args).dump(-1, /* to_json= */ true)))},
                              }},
                         },
                     })},
                };
                ...
                for (size_t i = 0; i < prefix.size() && i < full.size(); ++i) {
                    ...
                    if (prefix[i] == '<') {
                        ...
                        // The prefix and full strings diverge at <think> vs. <｜tool▁calls▁begin｜>, we avoid consuming
                        // the leading <.
                        continue;
                    }
    ...
    std::string apply(const nlohmann::ordered_json& messages, const nlohmann::ordered_json& tools,
                      bool add_generation_prompt,
    ...
    std::string apply(const chat_template_inputs& inputs,
        ...
        auto polyfill_tool_responses =
            opts.polyfill_tool_responses && has_tool_responses && !caps_.supports_tool_responses;
        ...
        auto needs_polyfills =
            opts.apply_polyfills && (false || polyfill_system_role || polyfill_tools || polyfill_tool_calls ||
                                     polyfill_tool_responses || polyfill_object_arguments || polyfill_typed_content);

        if (needs_polyfills) {
            actual_messages = json::array();

            auto add_message = [&](const json& msg) {
                if (polyfill_typed_content && msg.contains("content") && !msg.at("content").is_null() &&
                    msg.at("content").is_string()) {
            ...
            if (polyfill_tools) {
                adjusted_messages = add_system(
                    inputs.messages, "You can call any of the following tools to satisfy the user's requests: " +
                                         minja::Value(inputs.tools).dump(2, /* to_json= */ true) +
            ...
            for (const auto& message_ : adjusted_messages) {
                ...
                if (!message.contains("role") || (!message.contains("content") && !message.contains("tool_calls"))) {
                    throw std::runtime_error("message must have 'role' and one of 'content' or 'tool_calls' fields: " +
                                             message.dump());
                }
                ...
                if (polyfill_tool_responses && role == "tool") {
                    ...
                    if (message.contains("name")) {
                        obj["tool"] = message.at("name");
                    }
                    obj["content"] = message.at("content");
                    if (message.contains("tool_call_id")) {
                        obj["tool_call_id"] = message.at("tool_call_id");
                    }
                    message["content"] = "<tool_response>\n" + obj.dump() + "</tool_response>\n";
                    message.erase("name");
                }
        ...
        if (opts.define_strftime_now) {
            ...
            context->set("strftime_now",
                         Value::callable([now](const std::shared_ptr<minja::Context>&, minja::ArgumentsValue& args) {
