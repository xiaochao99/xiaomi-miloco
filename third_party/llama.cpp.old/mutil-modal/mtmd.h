...
MTMD_API mtmd_context* mtmd_init_from_file(const char* mmproj_fname, const struct llama_model* text_model,
...
// these getters/setters are dedicated functions, so you can for example calculate the hash of the image based on
...
MTMD_API int32_t mtmd_tokenize(mtmd_context* ctx, mtmd_input_chunks* output, const mtmd_input_text* text,
...
struct bitmap {
    ...
    bitmap(uint32_t nx, uint32_t ny, const unsigned char* data) { ptr.reset(mtmd_bitmap_init(nx, ny, data)); }
    ~bitmap() = default;
...
struct input_chunks {
    ...
    const mtmd_input_chunk* operator[](size_t idx) { return mtmd_input_chunks_get(ptr.get(), idx); }
};
