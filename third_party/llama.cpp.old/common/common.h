...
enum common_reasoning_format {
    ...
    COMMON_REASONING_FORMAT_DEEPSEEK_LEGACY,  // Extract thinking tag contents and return as
                                              // `message.reasoning_content`, or leave inline in <think> tags in stream
                                              // mode
...
struct common_params {
    ...
    int32_t n_ctx = 12288;          // context size
    int32_t n_batch = 4096;         // logical batch size for prompt processing (must be >=32 to use BLAS)
    int32_t n_ubatch = 1024;        // physical batch size for prompt processing (must be >=32 to use BLAS)
    int32_t n_seq_max = 128;        // maximum sequence length for prompt processing
    ...
    enum llama_split_mode split_mode = LLAMA_SPLIT_MODE_NONE;  // how to split the model across GPUs(single)
    ...
    bool lora_init_without_apply = false;  // only load lora to memory, but do not apply it to ctx (user can manually
                                           // apply lora later using llama_adapter_lora_apply)
    ...
    int32_t ppl_output_type =
        0;                   // = 0 -> ppl output is as usual, = 1 -> ppl output is num_tokens, ppl, one per line
    ...
    std::string embd_out = "";  // empty = default, "array" = [[],[]...], "json" = openai style, "json+" = same "json" +
...
ggml_opt_dataset_t common_opt_dataset_init(struct llama_context* ctx, const std::vector<llama_token>& tokens,
                                           int64_t stride);
