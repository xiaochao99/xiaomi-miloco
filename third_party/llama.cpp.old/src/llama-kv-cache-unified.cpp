...
llama_kv_cache_unified::llama_kv_cache_unified(const llama_model& model, layer_filter_cb&& filter, ggml_type type_k,
                                               ggml_type type_v, bool v_trans, bool offload, uint32_t kv_size,
                                               uint32_t n_seq_max, uint32_t n_pad, uint32_t n_swa,
...

    {
        ...
        LLAMA_LOG_INFO("%s: size = %7.2f MiB (%6u cells, %3d layers, %2u seqs), K (%s): %7.2f MiB, V (%s): %7.2f MiB\n",
                       __func__, (float)(memory_size_k + memory_size_v) / (1024.0f * 1024.0f), kv_size,
                       (int)layers.size(), n_seq_max, ggml_type_name(type_k),
...
llama_pos llama_kv_cache_unified::seq_pos_min(llama_seq_id seq_id) const { return cells.seq_pos_min(seq_id); }

llama_pos llama_kv_cache_unified::seq_pos_max(llama_seq_id seq_id) const { return cells.seq_pos_max(seq_id); }

llama_memory_state_ptr llama_kv_cache_unified::init_batch(const llama_batch& batch, uint32_t n_ubatch, bool embd_all) {
    ...
    do {
        ...
        return std::make_unique<llama_kv_cache_unified_state>(this, std::move(sbatch), std::move(heads),
                                                              std::move(ubatches));
    } while (false);
    ...
    // see if we need to defrag
    {
        ...
        if (!do_defrag && thold > 0.0f) {
            ...
            const float fragmentation =
                n_kv >= 2048 ? std::max(0.0f, 1.0f - (float(cells.get_used() + n_pad) / n_kv)) : 0.0f;
...
int32_t llama_kv_cache_unified::find_slot(const llama_ubatch& ubatch) const {
    ...
    if (debug > 0) {
        LLAMA_LOG_DEBUG("%s: n = %5d, used = %5d, head = %5d, size = %5d, n_swa = %5d\n", __func__, cells.used_max_p1(),
                        cells.get_used(), head, get_size(), n_swa);
        ...
        for (int s = 0; s < LLAMA_MAX_SEQ; ++s) {
            ...
            LLAMA_LOG_DEBUG("%s: min[%d] = %5d, max[%d] = %5d\n", __func__, s, cells.seq_pos_min(s), s,
                            cells.seq_pos_max(s));
        }
    }
    ...
    for (int s = 0; s < LLAMA_MAX_SEQ; ++s) {
        ...
        if (cells.seq_pos_min(s) <= seq_pos_max_rm[s]) {
            LLAMA_LOG_DEBUG("%s: purging positions [%d, %d] of sequence %d from KV cache\n", __func__,
                            cells.seq_pos_min(s), seq_pos_max_rm[s], s);
...
bool llama_kv_cache_unified::get_can_shift() const { return true; }

uint32_t llama_kv_cache_unified::get_size() const { return cells.size(); }

bool llama_kv_cache_unified::get_has_shift() const { return cells.get_has_shift(); }
...
ggml_tensor* llama_kv_cache_unified::get_k(ggml_context* ctx, int32_t il, uint32_t n_kv) const {
    ...
    return ggml_view_3d(ctx, k, hparams.n_embd_head_k, hparams.n_head_kv(il), n_kv,
                        ggml_row_size(k->type, hparams.n_embd_head_k), ggml_row_size(k->type, hparams.n_embd_k_gqa(il)),
                        0);
}
...
ggml_tensor* llama_kv_cache_unified::get_v(ggml_context* ctx, int32_t il, uint32_t n_kv) const {
    ...
    if (!v_trans) {
        // note: v->nb[1] <= v->nb[2]
        return ggml_view_3d(ctx, v, hparams.n_embd_head_v, hparams.n_head_kv(il), n_kv,
    ...
    // note: v->nb[1] > v->nb[2]
    return ggml_view_3d(ctx, v, n_kv, hparams.n_head_kv(il), hparams.n_embd_head_v,
...
ggml_tensor* llama_kv_cache_unified::cpy_k(ggml_context* ctx, ggml_tensor* k_cur, int32_t il, uint32_t head_cur) const {
    ...
    ggml_tensor* k_view = ggml_view_1d(ctx, k, n_tokens * hparams.n_embd_k_gqa(il),
...
ggml_tensor* llama_kv_cache_unified::cpy_v(ggml_context* ctx, ggml_tensor* v_cur, int32_t il, uint32_t head_cur) const {
    ...
    if (!v_trans) {
        v_view = ggml_view_1d(ctx, v, n_tokens * hparams.n_embd_v_gqa(il),
                              ggml_row_size(v->type, hparams.n_embd_v_gqa(il)) * head_cur);
    } else {
        // note: the V cache is transposed when not using flash attention
        v_view = ggml_view_2d(ctx, v, n_tokens, hparams.n_embd_v_gqa(il), (v->ne[1]) * ggml_element_size(v),
        ...
        for (int j = 0; j < n_tokens; ++j) {
            for (int i = 0; i < n_kv; ++i) {
                ...
                data[h * (n_kv * n_tokens) + j * n_kv + i] =
                    llama_relative_position_bucket(p0, ubatch->pos[j], hparams.n_rel_attn_bkts, false);
            }
        }
    }
}
...
ggml_tensor* llama_kv_cache_unified::build_rope_shift(const llama_cparams& cparams, ggml_context* ctx, ggml_tensor* cur,
                                                      ggml_tensor* shift, ggml_tensor* factors, float freq_base,
    ...
    const float yarn_attn_factor =
        model.arch == LLM_ARCH_DEEPSEEK2 ? 1.0f / (1.0f + 0.1f * logf(1.0f / freq_scale)) : cparams.yarn_attn_factor;
    ...
    if (ggml_is_quantized(cur->type)) {
        ...
        tmp = ggml_rope_ext(ctx, tmp, shift, factors, n_rot, rope_type, n_ctx_orig, freq_base, freq_scale,
                            yarn_ext_factor, yarn_attn_factor, yarn_beta_fast, yarn_beta_slow);

        tmp = ggml_cpy(ctx, tmp, cur);
    } else {
        // we rotate only the first n_rot dimensions
        tmp = ggml_rope_ext_inplace(ctx, cur, shift, factors, n_rot, rope_type, n_ctx_orig, freq_base, freq_scale,
                                    yarn_ext_factor, yarn_attn_factor, yarn_beta_fast, yarn_beta_slow);
    }
...
llm_graph_result_ptr llama_kv_cache_unified::build_graph_shift(const llama_cparams& cparams, ggml_context* ctx,
    ...
    for (const auto& layer : layers) {
        ...
        ggml_tensor* k =
            ggml_view_3d(ctx, layer.k, n_embd_head_k, n_head_kv, cells.size(),
                         ggml_row_size(layer.k->type, n_embd_head_k), ggml_row_size(layer.k->type, n_embd_k_gqa), 0);
...
llm_graph_result_ptr llama_kv_cache_unified::build_graph_defrag(const llama_cparams& cparams, ggml_context* ctx,
    ...
    for (uint32_t i = 0; i < ids.size(); ++i) {
        ...
        for (const auto& layer : layers) {
            ...
            ggml_tensor* view_k_src =
                ggml_view_2d(ctx, layer.k, n_embd_k_gqa, nm, ggml_row_size(layer.k->type, n_embd_k_gqa),
            ...
            if (cparams.flash_attn) {
                ...
                view_v_src = ggml_view_2d(ctx, layer.v, n_embd_v_gqa, nm, ggml_row_size(layer.v->type, n_embd_v_gqa),
                                          ggml_row_size(layer.v->type, n_embd_v_gqa * i));

                view_v_dst = ggml_view_2d(ctx, layer.v, n_embd_v_gqa, nm, ggml_row_size(layer.v->type, n_embd_v_gqa),
                                          ggml_row_size(layer.v->type, n_embd_v_gqa * id));
            } else {
                view_v_src = ggml_view_2d(ctx, layer.v, nm, n_embd_v_gqa, ggml_row_size(layer.v->type, cells.size()),
                                          ggml_row_size(layer.v->type, i));

                view_v_dst = ggml_view_2d(ctx, layer.v, nm, n_embd_v_gqa, ggml_row_size(layer.v->type, cells.size()),
...
llama_kv_cache_unified::defrag_info llama_kv_cache_unified::defrag_prepare(int32_t n_max_nodes) const {
    ...
    for (uint32_t i0 = 0; i0 < n_used; ++i0) {
        ...
        // GGML_ASSERT(nf == nh && "KV defrag bug: nf != nh kv移动目标不能是尚在使用的cell");
        if (nf != nh) LLAMA_LOG_ERROR("%s: KV defrag bug: nf != nh kv移动目标不能是尚在使用的cell\n", __func__);
...
void llama_kv_cache_unified::state_write(llama_io_write_i& io, llama_seq_id seq_id) const {
    ...
    for (uint32_t i = 0; i < cells.size(); ++i) {
        if (!cells.is_empty(i) && (seq_id == -1 || cells.seq_has(i, seq_id))) {
            // printf("llama_kv_cache_unified::state_write: cell %u has seq_id %d\n", i, seq_id);
            ++cell_count;
...
void llama_kv_cache_unified::state_read(llama_io_read_i& io, llama_seq_id seq_id) {
    ...
    // printf("llama_kv_cache_unified::state_read: cell_count = %u\n", cell_count);
    bool res = true;
...
void llama_kv_cache_unified::state_write_meta(llama_io_write_i& io,
                                              const std::vector<std::pair<uint32_t, uint32_t>>& cell_ranges,
                                              llama_seq_id seq_id) const {
    for (const auto& range : cell_ranges) {
        // printf("llama_kv_cache_unified::state_write_meta: range = [%u, %u), seq_id %d\n", range.first, range.second,
        //        seq_id);
        for (uint32_t i = range.first; i < range.second; ++i) {
            std::vector<llama_seq_id> seq_ids;

            // todo n_seq_max = LLAMA_MAX_SEQ;

            for (llama_seq_id cur = 0; cur < (int)LLAMA_MAX_SEQ; ++cur) {
                // printf("llama_kv_cache_unified::state_write_meta: checking cell %d for seq_id %d\n", i, cur);
                if (cur == seq_id || seq_id == -1) {
                    if (cells.seq_has(i, cur)) {
                        // printf("llama_kv_cache_unified::state_write_meta: cell %d has seq_id %d\n", i, cur);
            ...
            // printf("llama_kv_cache_unified::state_write_meta: pos = %d, n_seq_id = %u\n", pos, n_seq_id);
...
void llama_kv_cache_unified::state_write_data(llama_io_write_i& io,
                                              const std::vector<std::pair<uint32_t, uint32_t>>& cell_ranges) const {
...
bool llama_kv_cache_unified::state_read_meta(llama_io_read_i& io, uint32_t cell_count, llama_seq_id dest_seq_id) {
    if (dest_seq_id != -1) {
        ...
        for (uint32_t i = 0; i < cell_count; ++i) {
            ...
            // printf("llama_kv_cache_unified::state_read_meta: pos = %d, n_seq_id = %d\n", pos, n_seq_id);

            // 只有非共享的token才能够读，写的时候又未作限制
            if (n_seq_id != 1) {
            ...
            // printf("llama_kv_cache_unified::state_read_meta: ubatch[%u] pos = %d, seq_id = %d\n", i, pos,
            //        *ubatch.seq_id[i]);
        }
        ...
        // DEBUG CHECK: head_cur should be our first cell, head_cur + cell_count - 1 should be our last cell (verify
        // seq_id and pos values) Assume that this is one contiguous block of cells
    ...
    for (const auto& layer : layers) {
        ...
        if (k_size_row != k_size_row_ref) {
            LLAMA_LOG_ERROR("%s: mismatched key row size (%zu != %zu, layer %d)\n", __func__, k_size_row,
                            (size_t)k_size_row_ref, il);
            return false;
        }

        if (cell_count) {
            // Read and set the keys for the whole cell range
            ggml_backend_tensor_set(layer.k, io.read(cell_count * k_size_row), head * k_size_row,
                                    cell_count * k_size_row);
        }
    }
    ...
    if (!this->v_trans) {
        for (const auto& layer : layers) {
            ...
            if (v_type_i != v_type_i_ref) {
                LLAMA_LOG_ERROR("%s: mismatched value type (%d != %d, layer %d)\n", __func__, v_type_i, v_type_i_ref,
                                il);
                return false;
            }
            ...
            if (v_size_row != v_size_row_ref) {
                LLAMA_LOG_ERROR("%s: mismatched value row size (%zu != %zu, layer %d)\n", __func__, v_size_row,
                                (size_t)v_size_row_ref, il);
                return false;
            }

            if (cell_count) {
                // Read and set the values for the whole cell range
                ggml_backend_tensor_set(layer.v, io.read(cell_count * v_size_row), head * v_size_row,
                                        cell_count * v_size_row);
            }
        }
    } else {
        ...
        for (const auto& layer : layers) {
            ...
            if (cell_count) {
                ...
                for (uint32_t j = 0; j < n_embd_v_gqa; ++j) {
                    ...
                    ggml_backend_tensor_set(layer.v, io.read(cell_count * v_size_el), dst_offset,
                                            cell_count * v_size_el);
                }
            }
        }
    }
...
llama_kv_cache_unified_state::llama_kv_cache_unified_state(llama_kv_cache_unified* kv)
    : status(LLAMA_MEMORY_STATUS_SUCCESS), kv(kv) {
    n_kv = kv->get_size();
    head = 0;
}

llama_kv_cache_unified_state::llama_kv_cache_unified_state(llama_kv_cache_unified* kv, llama_context* lctx,
                                                           bool do_shift, defrag_info dinfo)
    : status(LLAMA_MEMORY_STATUS_SUCCESS), kv(kv), lctx(lctx), do_shift(do_shift), dinfo(std::move(dinfo)) {
    if (!do_shift && this->dinfo.empty()) {
        status = LLAMA_MEMORY_STATUS_NO_UPDATE;
    }
}

llama_kv_cache_unified_state::llama_kv_cache_unified_state(llama_kv_cache_unified* kv, llama_sbatch sbatch,
...
llama_memory_status llama_kv_cache_unified_state::get_status() const { return status; }
...
uint32_t llama_kv_cache_unified_state::get_n_kv() const { return n_kv; }
...
void llama_kv_cache_unified_state::set_input_k_shift(ggml_tensor* dst) const { kv->set_input_k_shift(dst); }

void llama_kv_cache_unified_state::set_input_kq_mask(ggml_tensor* dst, const llama_ubatch* ubatch,
