...
async def get_all_trigger_rules_v2(
    enabled_only: bool = Query(False, description="Whether to return only enabled rules"),
    current_user: str = Depends(verify_token)
):
    logger.info("Get all trigger rules v2 API called - User: %s, Enabled only: %s", current_user, enabled_only)
    trigger_rules: list[TriggerRuleV2] = await manager.trigger_rule_service.get_all_trigger_rules_v2(enabled_only)
    return NormalResponse(
        code=0,
        message=f"Trigger rules v2 retrieved successfully, total {len(trigger_rules)} records",
        data=trigger_rules
    )
...
async def update_trigger_rule_v2(
    ...
    logger.info("Update trigger rule v2 API called - User: %s, Rule ID: %s", current_user, rule_id)
...
async def delete_trigger_rule_v2(
    rule_id: str,
    current_user: str = Depends(verify_token)
):
    logger.info("Delete trigger rule v2 API called - User: %s, Rule ID: %s", current_user, rule_id)
    success = await manager.trigger_rule_service.delete_trigger_rule_v2(rule_id)
    if not success:
        raise BusinessException("Trigger rule v2 deletion failed")
    return NormalResponse(code=0, message="Trigger rule v2 deleted successfully", data=None)
